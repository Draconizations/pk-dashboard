<script lang="ts">
	import type { Member, Group } from "$lib/api/types"
    ...
    $: members.sort((a,b) => a.name?.localeCompare(b.name || "abcde") || 1)
    ...
    function addGroup(index: number) {
        if (value.filter(g => g.uuid === members[index].uuid)[0]) {
            value = value.filter(g => g.uuid !== members[index].uuid)
            return
        }

        value.push(members[index])
        value = value
    }
    ...
    function searchGroups() {
        ...
        for (let [index, member] of members.entries()) {
            if (focusIfMatch(index, member.id) === true) return
        }
        for (let [index, member] of members.entries()) {
            if (focusIfMatch(index, member.name) === true) return
        }
        for (let [index, member] of members.entries()) {
            if (focusIfMatch(index, member.display_name) === true) return
        }
    }
    ...
    function moveAround(event: KeyboardEvent) {
        ...
        if (event.key === "ArrowUp" || event.key === "ArrowDown") {
            ...
            if (event.key === "ArrowUp") {
                elements[active].active = false
                if (active === 0) {
                    elements[members.length - 1].scrollIntoView({
                        block: "nearest"
                    })
                    elements[members.length - 1].active = true
                    return
                } else {
                    elements[active - 1].scrollIntoView({
                        block: "nearest"
